...
- Полная энергия $E = E_k + U$ сохраняется с относительной погрешностью около $\sim 10^{-4}$
- Отсутствие дрейфа полной энергии подтверждает устойчивость метода Верле
...
Радиальная функция распределения показывает насколько вероятно найти другую частицу на расстоянии r от выбранной:
...
- При больших $r$ функция стремится к 1 (однородная среда)
...
- Средняя температура близка к заданной $T_0 = 1.0$, отклонение связано с конечным числом частиц и численными эффектами
